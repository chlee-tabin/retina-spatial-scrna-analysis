...
1. **Download the processed objects** from GEO and strip the `GSE322831_` prefix into `data/`:
   ...
   mkdir -p data
   for f in GSE322831_*.h5ad GSE322831_*.rds; do mv "$f" "data/${f#GSE322831_}"; done
   ```
   ...
   (`GSE322831_20260528_mouse_RPC_cr9.h5ad` is the full seven-library mouse object behind the interactive viewer; the figures use the E13.5–E16 subset. The GTF and genome files support re-alignment from raw reads. GEO's bundled `readme.txt` predates the mouse re-alignment; this table is current.)
...
2. **Generate figures** — each script is independent and runnable from anywhere inside the repository:
   ...
   Exception: `sfig18` and the SF23 half of `fig6h_sfig23` consume human/mouse Seurat objects that only the preprocessing scripts produce in-session — see *Reproducibility scope* below.
...
**Supported entry point: the GEO GSE322831 processed objects** (table above). Every Python figure script and the chick R figure scripts run directly from them.

**`scripts/preprocessing/` is the provenance record** of how those objects were made. It is published for transparency and is not fully re-runnable from public data alone: `01_chick_preprocessing.R` reads the raw Cell Ranger/cellsnp/vireo tree (raw reads are available from GEO; the aligned tree is not distributed), and `01`/`03`/`04` additionally read pre-publication intermediates (a 2024-08 merged chick object used for barcode cross-checks, and the merged human / integrated mouse Seurat objects, whose own derivation from public GSE138002/GSE234963/GSE246169 and GSE118614/GSE139904 data is retained in the archived analysis notebook).

The chain was re-executed end-to-end from the archived inputs (2026-03): the human export reproduced the original exactly (23,031 cells), the mouse subset exactly (25,202), and the chick pipeline to within ~1.4% of cells (83,915 vs 85,135) — the drift traces to unseeded `scDblFinder` doublet calls cascading through Harmony/Leiden. The GEO objects remain the objects of record for all downstream figures.
