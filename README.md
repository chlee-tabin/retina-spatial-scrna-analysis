...
4. **`scripts/analysis/`** — Non-figure analysis scripts (e.g. canonical Supplementary Table 1 / 2 generation)
5. **`docs/supplementary_tables/`** — Committed CSVs for the corrected manuscript Supp. Tables 1 (chick) and 2 (human) plus a schema README
...
#                     "glmGamPoi", "viridis", "ggforce", "ggh4x", "tictoc", "glue", "here"))
...
│   └── analysis/
│       └── area_significant_deg.R # Supp. Tables 1 (chick) / 2 (human) generator
│
├── docs/
│   └── supplementary_tables/      # Committed Supp. Tables 1 / 2 CSVs + schema
│       ├── README.md
│       ├── SuppTable1_chick_area_significant.csv
│       └── SuppTable2_human_area_significant.csv
│
└── notebooks/figures/
    ├── figure_legends.md
    └── methods_section.md
...
| SF16 + chick cluster-membership table | `sfig16_chick_spatial_clusters.py` | Python |
| SF17A-D | `sfig17_chick_signaling.py` | Python |
| SF18A-D | `sfig18_mouse_human_scores.R` | R |
| SF19 + human/mouse cluster-membership tables | `sfig19_mouse_human_clusters.py` | Python |
...
| Table | Script | Output |
|---|---|---|
| **Supp. Table 1** (chick area DEG, Fig. 6H) | `scripts/analysis/area_significant_deg.R` | `docs/supplementary_tables/SuppTable1_chick_area_significant.csv` |
| **Supp. Table 2** (human area DEG, Fig. S23) | `scripts/analysis/area_significant_deg.R` | `docs/supplementary_tables/SuppTable2_human_area_significant.csv` |
| Chick cluster-membership (with SF16) | `scripts/figures/sfig16_chick_spatial_clusters.py` | `figures/Tables/chick_cluster_members_sfig16.tsv` |
| Human / mouse cluster-membership (with SF19) | `scripts/figures/sfig19_mouse_human_clusters.py` | `figures/Tables/{human,mouse}_cluster_members_sfig19.tsv` |

The Supp. Table 1 / 2 area-DEG CSVs are committed under
`docs/supplementary_tables/` (see its README for schema and per-territory
counts). The cluster-membership tables under SF16 / SF19 are regenerated by
their figure scripts into `figures/Tables/` (gitignored); their filenames
deliberately omit a Supp-Table number to avoid colliding with the
revised-manuscript Supp. Table 1 / 2 namespace owned by the area-DEG CSVs.
