...
2. **Generate figures** — each script is independent and runnable from anywhere inside the repository:
   ...
   12 of the 14 figure scripts run from the GEO objects alone. The two exceptions are `sfig18` (all panels) and the SF23 half of `fig6h_sfig23` (its Figure 6H half runs from the GEO objects): they consume *in-session* human/mouse Seurat objects — objects that exist only as variables inside a running R session and are never written to disk. To produce them, open one R session and `source()` `scripts/preprocessing/03_human_preprocessing.R` before the figure script (`Rscript`, which starts a fresh session per script, cannot chain them); the mouse object additionally requires the CR9 re-alignment produced outside this repo. See *Reproducibility scope* below.
...
- **Mouse**: re-run 25,202 = original 25,202 — exact, for the superseded pre-CR9 lineage that `04` documents. The deposited CR9 `..._e13e16` object (26,505 cells) is validated separately by the re-alignment pipeline.
